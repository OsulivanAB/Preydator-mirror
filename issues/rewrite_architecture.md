...
| Vertical bar orientation — **signed off 2026-09-04.** Confirmed live end-to-end: fill gradient rotates correctly along the bar's length, percent text stays horizontal by deliberate product-owner choice (rotating it to match the stage label was tried, live-tested, reverted), stage label text rotates and reads correctly. See Decisions Log item 66. No known open items. | ✅ | ✅ |
...
61. **Text & Labels two-column layout built (2026-09-03) — the most concretely-specified of Decisions Log item 43's four batched Full-scope items, per the product owner's own choice of which to pick up first.** New `createEditBoxPairRow` in `UI/SettingsPanel.lua` puts a Prefix field and its corresponding Label field side by side on one row (left column at `PAIR_COLUMN_WIDTH = 260`, each editbox `PAIR_EDITBOX_WIDTH = 200`) instead of two stacked full-width rows — halves `buildTextLabelsCategory`'s vertical scroll length, matching the product owner's own mockup screenshot from item 43. Applied to all 7 existing pairs (Stage 1-4 Prefix/Label, Out of Zone, Ambush, Pack Ambush) — every `createEditBoxRow` call site in the file was a pair, none left over as a genuinely standalone field, so the old single-column helper is now fully replaced rather than kept alongside an unused duplicate; `buildEditBoxControl` was factored out first so the pair row doesn't duplicate the commit/revert (`OnEnterPressed`/`OnEscapePressed`) logic a second time. The dropdowns/font pickers/Font Size slider above the field list are untouched, still full-width, per item 43's original scope note. `luacheck`: 0 warnings/0 errors. **Not yet tested live** — the two longest labels ("Ambush Label (use {preyTargetName})", "Pack Ambush Label (use {packAmbushSourceName})") land in the right column with no verified upper bound on how much horizontal room the canvas actually has at that offset; flag if either looks clipped or crowds the category's edge.

62. **Vertical bar confirmed fully working live (2026-09-04); Text & Labels editbox round-trip closed out through several fixes; custom sound file Add/Remove UI built.** The gradient-rotation and percent-text-rotation-removal fixes (see item 66, added retroactively) both confirmed correct live -- Section 15's vertical-orientation row can be marked polished, not just "built, untested" anymore.

    Text field persistence, investigated after the product owner reported edited Stage Label text not surviving `/reload`: traced the full round-trip (`Settings.Set` -> `SettingsStore.Save` -> `PreydatorDB`, and back through `Load`/`MigrateAll`/`NormalizeAll`/`deepOverlayValid`) and found no code-level bug -- `MigrateAll`'s own already-migrated-data early return correctly no-ops for this profile shape, `NormalizeAll` doesn't touch `text.*` fields at all. Rather than guess-patch working code, ported a real gap found by comparing against the old codebase's own `CreateTextInput` helper (Modules/Settings.lua:866-875): every commit now re-reads via `getter()` and re-displays that, instead of trusting the just-typed text, surfacing any silent normalization/rejection immediately in the box itself. Combined with the earlier `OnEditFocusLost` commit-on-blur fix (item 58's companion), the product owner confirmed live: "the stage text is holding." Root cause of the original persistence report was never conclusively isolated to one line, but is very likely explained by the same missing-commit-on-blur gap this session's earlier fix already closed -- not a live open item since it stopped reproducing.

    Separately, empty (never-customized) text boxes were confirmed showing blank instead of their real default text (e.g. "Blood in the Shadows") -- root cause not pinned down (editboxes are created after PLAYER_LOGIN, when Settings should already be loaded, so the exact timing gap is unconfirmed), but closed by extending the same `Settings.Subscribe`-driven refresh pattern already built for slider labels (Decision 60) to editboxes too, guaranteeing self-correction on any settings change regardless of the original cause. Guarded against yanking text out of a box the player is actively typing in (`EditBox:HasFocus()` check), since `Settings.Subscribe` fires for any settings change, not just the box's own key.

    Also added, both product owner requests the same session: a second "Restore Default Names" button directly in Text & Labels (top-right, above the scroll area, reusing the existing `NAME_FIELD_KEYS`/`restoreDefaults` the Advanced-tab button already used -- that one left in place, not removed); and the custom sound file Add/Remove UI (`sound.custom_file_names` already had matching data-side plumbing waiting -- `GROWABLE_LIST_FIELDS`, `PROTECTED_SOUND_FILENAMES` -- since before this session). New `SoundsRuntime.AddCustomSoundFile`/`RemoveCustomSoundFile`, business logic kept out of `UI/SettingsPanel.lua` per this file's own "thin UI caller" rule, with normalize/validate rules ported directly from the old codebase's `NormalizeSoundFileName` (trim, lowercase for case-insensitive matching, strip the sound folder prefix if a full path is pasted, reject subdirectories, auto-append `.ogg`). UI is an editbox + Add File/Remove File buttons in the Advanced category, matching where the old app had it (product owner's own reference pointer). `luacheck`: 0 warnings/0 errors across all touched files. **Not yet tested live.**

63. **Default prey icon reappearing mid-hunt: root-caused via a new always-on passive trace, and closed as a known, accepted limitation rather than a bug to keep chasing (2026-09-04).** Product owner reported the icon "randomly" reappearing, too briefly to react to live with `/pd pinspect`. Built `WidgetAdapter.GetSuppressionTrace()` (20-entry ring buffer, always recording -- not opt-in like the nameplate trace, since these are rare state-transition events, not a per-frame flood) plus `DiagnosticsRuntime.BuildIconSuppressionInspectReport`/`/pd iinspect`, recording every Setup/OnShow fire, actual Hide()/Show() call, and combat-lockdown block, each tagged with `InCombatLockdown()` state at that moment.

    First real capture confirmed the mechanism precisely: Blizzard showed the icon while already in combat, followed by five consecutive blocked re-suppression attempts, still in combat when the trace window ended -- the icon stays visible for as long as combat continues past that point, not a brief flicker. Checked whether the `InCombatLockdown()` guard itself could be loosened before proposing anything: it's not overcautious rewrite-only guessing -- the old (pre-rewrite) codebase's `ApplyDefaultPreyIconVisibility` has the identical hard block, with its own explicit comment ("Taint safety: do not mutate Blizzard prey widget frames during combat"), confirming this is a real, deliberate WoW protected-frame restriction both codebases independently arrived at, not something safe to remove. Presented the product owner two options (leave as-is vs. an unverified live experiment loosening only `SetAlpha` during combat) -- **decision: leave as-is**, documented in `WidgetAdapter.lua`'s own comment at the block so this doesn't get re-investigated as if it were still unexplained. `/pd iinspect` stays in the addon permanently as a real diagnostic tool, not temporary scaffolding -- useful if a *different* icon-visibility bug is ever reported, or if this decision is ever revisited. `luacheck`: 0 warnings/0 errors across all touched files.

64. **Text & Labels blank-box fix attempt REVERTED (2026-09-04) -- made the bug worse, not better.** Product owner reported the real pattern precisely: after a fresh `/reload`/relog, every box (custom text or default alike) read blank, and stayed blank until *any* unrelated `Settings.Set` happened to fire (e.g. clicking "Restore Default Names"). First attempt: added `canvas:SetScript("OnShow", refreshEditBoxValues)` to `buildTextLabelsCategory`, matching `buildBarColorsCategory`'s identical pattern for its color swatches. **Confirmed live this made it worse** -- afterward no box showed any text at all, not even after clicking Restore Default Names (which previously did work). Reverted immediately rather than guess further on top of a regression -- see item 65 for the real root cause and fix, found by getting actual diagnostic data instead of guessing a third time.

65. **Text & Labels blank-box bug: actual root cause found via live diagnostic prints, and fixed (2026-09-04) -- not a data or timing bug at all.** After item 64's regression, added a temporary print inside `buildEditBoxControl` logging `getter()`'s return value at the exact moment each box was created. Result: every single box (all 14) printed its **correct** value at creation -- empty prefixes, "Scent in the Wind"/"Blood in the Shadows"/etc. for labels, matching `PreydatorDB`'s raw stored data exactly (cross-checked via the product owner's own `/run` prints of `PreydatorDB.profiles[...].text.stage_suffix[2]` and `Settings.Get`, both correct). Yet the boxes were still visually blank on screen. Extended the diagnostic further -- a delayed check 3 seconds after creation reading `editBox:GetText()` (the box's own live internal content, not the setting), `IsShown`, and `GetAlpha` -- and every box came back with `GetText()` exactly matching `getter()`, `IsShown=true`, `GetAlpha=1`. Every measurable internal property was correct; only the visual rendering was wrong.

    This is a known WoW `EditBox` quirk, not a bug in this addon's own logic: text set via `SetText()` before the box's true on-screen width is settled by the frame's own layout pass can leave the box's internal scroll/cursor view stuck past the visible area, even though the stored text, shown-state, and alpha are all completely correct -- the text is there, just scrolled out of the visible viewport. `SetCursorPosition(0)` after `SetText()` is the standard fix (forces the visible view back to the start of the string). Applied everywhere `buildEditBoxControl` calls `SetText` -- initial creation, `OnEnterPressed`, `OnEscapePressed`, `OnEditFocusLost` -- and to `refreshEditBoxValues`'s own `SetText` call too, for consistency across every code path that can (re)populate a box. Temporary diagnostic prints removed once the cause was confirmed. `luacheck`: 0 warnings/0 errors. **CONFIRMED LIVE (2026-09-04)** -- product owner confirmed all text is working correctly, including custom text surviving a reload. No known open items on this bug.

66. **Retroactive documentation: the vertical bar gradient-rotation and percent-text-rotation fixes never got their own Decisions Log entry (2026-09-04) -- correcting item 62's mis-citation of "item 60," which doesn't cover this.** Both changes are already shipped (commit `c74fb36`) and covered in `CHANGELOG.md`, just missing from this log. For the record: `UI/BarFrame.lua`'s `applyFillTexture` never rotated the fill texture's UV coordinates for vertical mode -- only the fill region's size/position went vertical (`applyFill`), not the gradient direction, so it visually ran left-to-right across the bar's narrow width instead of along its length. Fixed with an 8-value `SetTexCoord` rotation, oriented so the gradient's start lands at whichever edge `bar.vertical_fill_direction` fills from (mirroring how the horizontal case already behaves unrotated). Percent text was then also tried rotated to match the stage label text (same `CENTER`-anchor-then-`SetRotation` pattern, `bar.vertical_text_side`-driven) -- live-tested, and the product owner decided against it: percent text stays horizontal in every placement, including "inside" in vertical mode, unlike the stage label text beside it which does still rotate. **Vertical bar orientation signed off in full (2026-09-04)** -- Section 15's row updated from "built, untested" to fully polished, no known open items.

67. **Sound amplification built (2026-09-04) -- the last open sub-item from Decisions Log item 43's batched Full-scope list (Section 5f item 2).** Modeled directly on the Better Fishing addon's "Enhance Sounds" feature, per the product owner's own reference pointer: while a Preydator alert plays, briefly silence `Sound_EnableAmbience`/`Sound_MusicVolume`/`Sound_EnablePetSounds`, force-enable `Sound_EnableSFX`/`Sound_EnableAllSound`/`Sound_EnableSoundWhenGameIsInBG`, and push `Sound_SFXVolume`/`Sound_MasterVolume` to a user-chosen 0-1 scale, restoring the player's own cached values afterward -- these are the player's own client-side sound CVars, nothing protected/secret, so no taint surface. New `Core/Adapters/SoundAdapter.lua` functions `BoostVolume(scale)`/`RestoreVolume()` are the sole CVar boundary (per this file's "Adapters are the only Blizzard API boundary" rule); `SoundsRuntime.lua`'s `playPath()` calls them, gated on new settings `sound.amplify_enabled` (default `false` -- opt-in, since it touches the player's real volume mix, not just Preydator's own playback) and `sound.amplify_scale` (default `1`, validated 0-1 in `SettingsRuntime.NUMERIC_RANGES`). **Diverges from Better Fishing's own model on purpose, not a straight port:** Better Fishing boosts for the whole cast-to-catch window (could be a long, indeterminate wait), which would mute the player's music/ambience for a Preydator hunt's entire multi-minute duration -- too intrusive for how long Preydator alerts actually last. Instead, `SoundAdapter` ref-counts the boost (`boostRefCount`) and `SoundsRuntime` schedules a fixed `AMPLIFY_RESTORE_DELAY_SECONDS = 4` restore per play via `C_Timer.After` (long enough to cover Preydator's own ~2-3s alert clips; PlaySoundFile has no real playback-finished callback to key off instead) -- the ref-count is what keeps two alerts overlapping within that window from having the first one's restore clobber the second's still-needed boost. New checkbox + 0.05-step slider ("Amplify Alert Sounds" / "Amplify Volume") added to `UI/SettingsPanel.lua`'s Sound & Alerts category using the existing `registerCheckbox`/`registerSlider` helpers, no new UI plumbing needed. `/pd sinspect` now also reports `amplify_enabled`/`amplify_scale` alongside the existing channel/cooldown line, matching this file's established "every relevant setting visible in inspect output" precedent. `luacheck`: 0 warnings/0 errors across all 6 touched files (`Core/Adapters/SoundAdapter.lua`, `Core/Adapters/SettingsStore.lua`, `Core/Runtime/SoundsRuntime.lua`, `Core/Runtime/SettingsRuntime.lua`, `Core/Runtime/DiagnosticsRuntime.lua`, `UI/SettingsPanel.lua`). No warning-561 concern anywhere. **Not yet tested live** -- next step is confirming in-game that toggling the checkbox actually changes perceived alert volume, and that the player's normal music/ambience mix returns correctly ~4s after a played alert.

68. **Sound amplification bug fix (2026-09-04, same day): the boost math was setting an absolute target, not actually amplifying.** Product owner tested item 67 immediately -- `sound.amplify_enabled=true`, `sound.amplify_scale=0.5` -- and reported no audible amplification. Root cause: `SoundAdapter.BoostVolume` was calling `SetCVar(cvar, scale)` directly, treating `scale` as the CVar's new absolute value -- for a player whose normal `Sound_SFXVolume`/`Sound_MasterVolume` was already at or above `0.5` (the overwhelmingly likely case, since WoW's own default is `1.0`), this made Preydator's alert the SAME volume or QUIETER than normal, not louder; the only genuinely audible part of the feature (silencing ambience/music/pet sounds) doesn't scale with the slider at all, so a mid-range scale value produced no perceptible change. **Fixed:** `scale` is now an ADDITIVE boost on top of the player's own live `GetCVar` reading at boost time, clamped to WoW's `1.0` ceiling -- the boosted volume can now never be lower than the player's current volume, and `1` (the default) always forces both to full, same end behavior as before for that one value. Also added `SoundAdapter.GetLastBoostSnapshot()` (requested scale + before/after SFX/Master values from the most recent boost) surfaced in `/pd sinspect` as a new `last volume boost:` line -- built specifically because this bug was only diagnosable with real before/after numbers, not from settings values or perceived loudness alone, matching this project's established pattern (`/pd ninspect`, `/pd iinspect`) of building a diagnostic the moment a bug turns out to be invisible without one. `luacheck`: 0 warnings/0 errors across all 6 touched files. **Not yet re-tested live** -- next step is confirming the product owner now hears an actual increase with `amplify_scale=0.5` (or any value), and that `/pd sinspect`'s new line shows the expected before/after numbers.

69. **A fourth zone false negative found live (2026-09-04, Voidstorm) -- this time isOnMap itself was wrong for the whole encounter, not a pre-filter heuristic bug like the three before it, so the fix is a different shape than items 29/32/35/36.** Product owner pasted a `/pd qinspect` dump from a PvP-optional sub-zone inside Voidstorm: `isOnMap=false`, `zoneID=nil`, `zoneName=nil`, for a hunt with a live, in-progress objective (`0/1 Hunt your Prey`, `isOnQuest=true`) -- and confirmed Blizzard's own default prey icon stays visible there the whole time (only observable with `general.disable_default_prey_icon` off), meaning Blizzard's own widget system considers the player legitimately in a prey-hunt context even though `C_QuestLog.GetInfo(...).isOnMap` says otherwise. This directly contradicts item 36's working assumption that isOnMap is fully authoritative -- confirmed here it is not, for at least this one sub-zone shape (a PvP-optional layer over an open-world zone). **Rejected reintroducing any map-hierarchy pre-filter** (item 36 already removed that exact class of heuristic after three separate live false negatives of its own, items 29/32/35) -- instead added a second, independent Blizzard signal: new `WidgetAdapter.IsPreyWidgetVisible()` reads whether Blizzard's own prey-hunt widget frame currently wants to be shown, the same signal the product owner used to catch the bug in the first place. Two read paths depending on `general.disable_default_prey_icon`: when off (default), a direct `iconFrame:IsShown()` read is fully trustworthy since Preydator never touches the frame; when on, Preydator's own suppression continuously `Hide()`s the frame, which would make a direct `IsShown()` read always false regardless of Blizzard's real intent -- falls back instead to "has Blizzard tried to re-show it within the last `WIDGET_RECENTLY_SHOWN_WINDOW = 6` seconds" (`lastShownAt`, updated by the existing `OnShow` hook from item 31), which stays continuously current specifically because Blizzard's own widget layout keeps re-showing the frame against Preydator's `Hide()` call every cycle while genuinely active -- the exact behavior that made an always-on `OnShow` hook necessary for suppression in the first place, now reused as a live signal instead of only a suppression trigger. New `PreyContextRuntime.ResolveQuestOnMap(questID)` is the single source of truth combining both signals (only overrides a CONFIRMED `false`, never guesses on `nil`) -- both call sites that used to call `QuestApiAdapter.GetQuestIsOnMap` directly (this file's own zone-gating step, and `AlertsRuntime`'s true-ambush/Mob-Scanner checks) now go through it instead, so the one fix applies to the bar AND both sound triggers at once rather than needing three separate patches. `/pd qinspect` now shows both `isOnMap` (raw) and `resolvedIsOnMap` (post-fallback) side by side, so a future divergence like this one is visible directly instead of requiring a fresh diagnostic to be built first. `luacheck`: 0 warnings/0 errors across all 4 touched files (`Core/Adapters/WidgetAdapter.lua`, `Core/Runtime/PreyContextRuntime.lua`, `Core/Runtime/AlertsRuntime.lua`, `Core/Runtime/DiagnosticsRuntime.lua`). **Not yet tested live** -- next step is the product owner re-checking `/pd qinspect` in that same Voidstorm sub-zone to confirm `resolvedIsOnMap=true` now, and confirming the bar/ambush sound/Mob Scanner all actually work there.

70. **Item 69's fix retested live, came back `resolvedIsOnMap=false` -- the fallback didn't fire, plus a widened widget-container scan added as an unconfirmed follow-up (2026-09-04).** Product owner re-ran `/pd qinspect` in the same Voidstorm sub-zone; `isOnMap=false` again, and `resolvedIsOnMap=false` too, meaning `WidgetAdapter.IsPreyWidgetVisible()` didn't find the widget frame at all. Working hypothesis: `captureLiveFrames()` only ever scanned one widget container (`UIWidgetPowerBarContainerFrame`), while `WidgetAdapter.DebugWidgetState()` (the temp diagnostic from the original stage-tracking investigation, 2026-08-28) already checks two additional container names -- built specifically because "current-patch widget container names had already proven stale for other assumptions" that session. If this specific PvP-optional sub-zone renders the prey icon through one of the other two containers, `captureLiveFrames` would never see it regardless of how correct `IsPreyWidgetVisible`'s own logic is. **Widened `captureLiveFrames()` to scan all three container names** (new shared `WIDGET_CONTAINER_NAMES` constant, also de-duplicating `DebugWidgetState`'s own copy of the same list into one source of truth) -- a free, read-only, purely additive change with no behavior-change risk if this isn't the actual cause. **Not confirmed as the real fix**: before it could be re-tested in the failing state, the product owner progressed to the hunt's final stage and reported `isOnMap=true`/`resolvedIsOnMap=true` on its own -- meaning either the player physically moved out of the problem sub-area as the hunt progressed (isOnMap self-corrected, unrelated to this fix), or the widened scan would have caught it had it still been failing; no way to distinguish the two from this session's data. `luacheck`: 0 warnings/0 errors. **Genuinely open** -- next real occurrence, `/pd qinspect`'s `resolvedIsOnMap` line answers it directly: `true` confirms the widget fallback (and this widening) worked, `false` again means the widget system itself isn't finding a matching frame in that sub-zone at all, at which point the honest conclusion is a real Blizzard-side limitation for this specific zone shape, not something to keep patching blindly.

71. **New passive `/pd zinspect` trace, built proactively (2026-09-04) after item 70's retest window closed before it could be caught mid-failure.** The product owner asked directly whether something like `iinspect`'s always-on suppression trace should exist for zone resolution too -- yes, and for exactly the reason item 70 couldn't be fully confirmed: manual `/pd qinspect` checks depend on running the command at the right moment, and the Voidstorm failure resolved itself before that happened a second time. New `PreyContextRuntime.ResolveQuestOnMap` records a bounded (`ZONE_TRACE_LIMIT = 20`) trace entry every time the raw `isOnMap` answer is `false` -- deliberately not logging the routine `true` case too, since a hunt's ~2s refresh tick would flood a small ring buffer with nothing useful otherwise. Each entry carries both `isOnMap`/`resolvedIsOnMap` and a new `WidgetAdapter.GetVisibilityDebugInfo()` snapshot (`iconFrameFound`, `desiredSuppression`, `directShown`, `lastShownAtAge`) -- specifically so a future "fallback still says false" report is diagnosable from what's already recorded (was a frame ever found at all? in which state?) rather than needing to reproduce the exact failing moment live a second time. Exposed via `DiagnosticsRuntime.BuildZoneInspectReport` / new `/pd zinspect [bs]` command, added to `Core/SlashCommands.lua`'s `COMMANDS` table and `USAGE` string alongside the existing inspect commands. `luacheck`: 0 warnings/0 errors across all 4 touched files (`Core/Adapters/WidgetAdapter.lua`, `Core/Runtime/PreyContextRuntime.lua`, `Core/Runtime/DiagnosticsRuntime.lua`, `Core/SlashCommands.lua`). **Not yet tested live** -- next occurrence of the Voidstorm-style zone issue, `/pd zinspect` should show exactly what `captureLiveFrames()` found (or didn't) at the moment `isOnMap` went false.

72. **`/pd zinspect` immediately proved its worth: a real 20-entry trace showed the widget WAS found the whole time, but `WIDGET_RECENTLY_SHOWN_WINDOW`'s 6s guess was too short for Blizzard's actual re-show cadence (2026-09-04).** Product owner ran `/pd zinspect` in the field (questID 91251) right after item 71 shipped -- `iconFrameFound=true` on every single one of the 20 recorded entries, meaning the widget-container-scan widening (item 70) was never actually the problem: the frame was found consistently throughout. The real defect was in `IsPreyWidgetVisible`'s suppressed-icon fallback path: `lastShownAtAge` climbed steadily from ~3.7s past 6s up to ~13.8s before `OnShow` fired again (observed gap ~14.6s between re-triggers, not the assumed ~2s refresh-tick cadence item 69 guessed at) -- `resolvedIsOnMap` correctly followed the 6s window as coded, flickering `false` for roughly 8 of every ~15 seconds even though the player never left the zone and the widget stayed genuinely present the entire time. **Fixed: `WIDGET_RECENTLY_SHOWN_WINDOW` raised 6 -> 20**, comfortably clearing the observed ~14.6s gap with margin -- a data-derived correction, not another guess, made possible specifically because item 71's trace captured the real cadence instead of requiring it to be inferred blind. Both comments that stated the wrong "re-shows every refresh tick" assumption (`WIDGET_RECENTLY_SHOWN_WINDOW`'s own comment and `IsPreyWidgetVisible`'s) were corrected to cite the actual observed behavior instead. `luacheck`: 0 warnings/0 errors. **Not yet re-tested live** -- next occurrence, `/pd zinspect` should show `resolvedIsOnMap=true` continuously (no more mid-cycle flicker to `false`) as long as `lastShownAtAge` stays under 20s between real `OnShow` events; if Blizzard's real cadence ever exceeds that with a wider gap than this one sample showed, the trace will show it directly and the fix is a one-line number change, not a redesign.

73. **Item 72's window widening retested live, disproven outright -- two real `/pd zinspect` traces (not one) showed Blizzard's `OnShow` re-trigger has no predictable cadence at all, and a combat-lockdown interaction was found and fixed as a real, separate gap (2026-09-04).** Product owner ran `/pd zinspect` twice more in the same Voidstorm sub-zone. First trace: `lastShownAtAge` was `nil` for 14 straight entries (no `OnShow` had fired yet at all since the frame was captured), then jumped to fresh (~0s) for the remaining 6. Second trace, ~64s later: `lastShownAtAge` climbed continuously from 47.5s to 73.5s with **zero** new `OnShow` events across the entire ~26s sample -- directly disproving item 72's "~15s cadence" theory; the real gap can run well past a minute with no known upper bound, so no fixed window size is a real fix, only a bigger guess. Also found in this round: `desiredSuppression=true` on every entry looked like a contradiction with the product owner's own memory of the setting (they believed "Hide Blizzard's Prey Icon" was off) -- resolved by checking `/pd iinspect`'s independent `settings disable_default_prey_icon=` line directly, which confirmed the setting genuinely was `true` (no settings-sync bug; the product owner's own recollection was simply out of date). That same `/pd iinspect` output surfaced a real, previously-unaccounted-for factor: repeated `suppress_blocked_combat` entries (`inCombat=true`) -- the product owner was fighting the prey mob for much of the window, and `applyDesiredSuppression()` cannot call `Hide()` at all while `InCombatLockdown()` is true (an existing, accepted restriction, Decisions Log item 63) -- meaning `IsPreyWidgetVisible` had a real blind spot: while blocked-by-combat, the frame is left completely untouched by Preydator, so a direct `IsShown()` read would have been fully trustworthy, but the function never checked combat state and always fell through to the (now-disproven) `lastShownAt` recency guess regardless. **Fixed:** `IsPreyWidgetVisible()` now treats a direct `IsShown()` read as trustworthy whenever suppression isn't actually touching the frame -- either the setting is off, or it's on but blocked by combat lockdown -- checked via `InCombatLockdown()` before falling through to the recency fallback. The recency fallback (`lastShownAt`/`WIDGET_RECENTLY_SHOWN_WINDOW`) is kept, not removed -- it's harmless (can only make the function more permissive, never less) and still catches the brief window right after a genuine `OnShow` -- but its own comment and this entry are both explicit that it is NOT a real fix for the "suppressed AND out of combat AND no recent OnShow" case, which has no known reliable signal at all right now. New `WidgetAdapter.GetVisibilityDebugInfo()` field `inCombat`, threaded through `PreyContextRuntime`'s zone-resolution trace and `/pd zinspect`'s output, so combat state can be directly correlated with outcomes on the next real occurrence instead of needing a separate `/pd iinspect` cross-reference. `luacheck`: 0 warnings/0 errors across all 3 touched files. **Not yet tested live.** **Honest status, revised same day:** the product owner pointed out combat is one of the core ways a hunt actually progresses (fighting the prey/mobs), not an incidental side state -- meaning "actively hunting in this zone" and "in combat" overlap heavily in practice, not just occasionally. That raises real confidence the combat-lockdown fix covers the bulk of the practical problem, not a narrow edge case, since a large share of the time this bug matters (mid-fight, prey nearby) is exactly the window it now handles correctly via a direct, trustworthy `IsShown()` read. The genuinely unsolved gap narrows to specifically "suppressed AND out of combat AND no recent OnShow" -- plausibly a smaller slice of real playtime in this zone shape (e.g. traveling between mobs, or right after combat ends but before the next pull) rather than the dominant case. Still not proven without live data -- if `/pd zinspect` next time still shows sustained `resolvedIsOnMap=false` stretches with `widgetInCombat=false` throughout, that narrower remaining gap is a genuine, currently-unsolved limitation worth documenting as such, not something to keep guessing at with more timing heuristics.

74. **New "confirmed active" latch, direct product owner diagnosis (2026-09-04): the bar only appeared in brief windows right after a container/ambush/Pack Ambush/Exploding Corpse Snakes event, then vanished again -- "if we can show it during those brief windows why can we not show it the entire time."** Correct root cause: those four trigger events all tend to coincide with combat starting (an ambush/Mob Scanner nameplate match typically pulls the mob into combat), which is exactly the window item 73's combat-lockdown fix makes reliable -- but the moment that specific window closes (combat ends, or the recency fallback expires), `ResolveQuestOnMap` re-derives zone status from scratch on the very next ~2s tick with no memory of the recent confirmation, flickering the bar/sounds off again even though the player never actually left. **Fix:** a bounded "confirmed active" latch -- whenever `ResolveQuestOnMap` returns `true` for a quest (via `isOnMap` itself or the widget-visible fallback), the moment is remembered (`markConfirmedTrue`); if a later call for the SAME quest finds both isOnMap and the widget check false, it now checks whether that quest was confirmed true within the last `CONFIRMED_ACTIVE_WINDOW_SECONDS = 120` seconds and, if so, still returns `true` rather than flipping off immediately. Deliberately bounded, not sticky for the rest of the hunt -- an unbounded latch would reintroduce the exact false-positive shape that caused the old pre-rewrite codebase's Eversong Woods bug (items 29/32/35/36) if the player travels far from the prey zone while still holding the same quest. 120s is explicitly a starting value, not a confirmed-safe one -- picked to comfortably bridge normal gaps between combat encounters within one continuous hunt (the default ambush cooldown is 60s, so confirming events can naturally land tens of seconds apart) without staying latched for minutes after genuinely leaving; a single constant to retune either direction if live testing shows it wrong. `/pd zinspect` now shows `resolvedVia` (`widget`/`latch`/`none`) and `latchAgeSeconds` per entry, so it's directly visible which mechanism resolved (or failed to resolve) each moment. `luacheck`: 0 warnings/0 errors across both touched files (`Core/Runtime/PreyContextRuntime.lua`, `Core/Runtime/DiagnosticsRuntime.lua`). **Not yet tested live** -- next occurrence, the bar/sounds should stay continuously active through normal combat gaps in that zone instead of flickering with each event; watch for the opposite failure mode too (staying lit noticeably after actually leaving the area), which would mean the window needs shortening, not lengthening.

75. **Old-code cleanup pass, first half (2026-09-04): six fully-superseded off-.toc monolith files removed, plus a small locale audit and two real fixes.** Product owner asked to sweep the tree for old code no longer needed, including locale files. Verified via grep (not assumed) that no active `.toc`-listed file references any of the six old `Modules/*.lua` files (`DebugInspect.lua`, `EditMode.lua`, `HuntScanner.lua` (5045 lines), `ReportWindow.lua`, `Settings.lua` (~3600 lines), `SlashCommands.lua`) -- every hit for their `RegisterModule`/`GetModule` names traced back to the old files calling each other, never to active code. `Modules/Settings.lua` had been previously protected by a `.toc` comment citing it as a live reference for `issues/bar_rendering_research.md` -- checked that doc directly: its citations point at `D:\Dev\PreydatorLive\Modules\Settings.lua`, a separate, permanent worktree holding the unmodified `main` branch, not this branch's own copy -- so removing this branch's copy doesn't affect that research doc's validity. All six `git rm`'d; `Preydator.toc`'s trailing comment block rewritten to record why, replacing the now-stale "don't remove yet" reasoning.

    Locale audit: built the true set of currently-called `L(...)` keys by extracting every literal-string call site across all 26 active `.toc`-listed files (found and had to correct for two indirect-reference misses along the way -- `SettingsStore.lua`'s `LOCALIZED_STRING_DEFAULTS`/`LOCALIZED_STAGE_SUFFIX_KEYS` tables pass keys through a local `localize()` wrapper, not a direct `L("...")` call, so a naive grep missed 7 real keys at first), landing on 200 confirmed-used keys. Diffed against every `Locales/*.lua` file's defined keys:
    - `deDE`/`esMX`/`frFR`/`itIT`/`ptBR`/`zhTW` (3 keys each -- Normal/Hard/Nightmare difficulty names): all 3/3 still used, nothing to change.
    - `esES`: 1 real bug found, not just staleness -- `L["AMBUSH"]` (no trailing punctuation) no longer matches the code's actual lookup key, which is `"AMBUSH: "` (colon-space, since `SettingsStore.lua`'s `ambush_prefix` default is used directly as a bar-text prefix with no separate punctuation added). Per this project's own Localization Discipline rule ("if a translation appears to break runtime matching, fix the parsing/normalization -- never edit the translator's wording"), fixed the KEY to `"AMBUSH: "` and extended the Spanish VALUE with the matching `": "` (a punctuation completion needed for the string to render correctly as a prefix, e.g. "EMBOSCADA: Kursak the Coiled" instead of running the words together -- not a wording change to jaestevan's actual translation "EMBOSCADA").
    - `ruRU`: 1 genuinely dead entry, `L["N/H/Ni"] = "о/в/к"` -- built for the old Warband panel's difficulty-initials header (2.2.0-era feature), which is permanently out of scope per `CLAUDE.md` Section 2 (Currency/Warband functionality). Removed; the 3 difficulty-name translations (credited @reysonk) are untouched.
    - `enUS.lua`: entirely regenerated. The old version was 100% inert (its whole body sat inside a `--[[ ]]` block comment, confirmed before touching it) but listed ~500 example keys from the pre-rewrite Currency Tracker/Warband panel/old Options page -- actively misleading as a "translation guide" since none of those keys exist in this codebase and dozens of real current keys (Sound & Alerts, Hunt Scanner, Bar Colors, etc.) were entirely absent. Replaced with all 200 currently-used keys, alphabetized, same inert-guide format (still wrapped in the block comment, zero functional change -- this file has never executed any of its content).
    - `koKR` (487 defined, 70 used, 417 orphaned) and `zhCN` (365 defined, 62 used, 303 orphaned): both large, real, credited translations (koKR: "Elnarfim"; zhCN: "@huchang47") built almost entirely for old-codebase features that no longer exist in the rewrite at all (Currency Tracker, Warband panel, the pre-rewrite Options page structure). Flagged directly to the product owner rather than deciding unilaterally, given the standing rule against discarding native-speaker translation work -- **product owner chose to strip both to their still-used subset**, matching the treatment already applied to `esES`/`ruRU`/`enUS` above. Filtered programmatically (PowerShell, not hand-edited) by key against the same 200-key used set, preserving every surviving translated value byte-for-byte and the file's header/locale-guard lines untouched; script and process documented so this is reproducible, not a one-off manual edit. Reduced `koKR` to 70 keys, `zhCN` to 62.
      - Along the way, found and resolved a real pre-existing bug in `zhCN.lua`, not introduced by this cleanup: the key `"Vertical"` was assigned two DIFFERENT translations at two separate points in the original file (`"布局"` ["layout"] and `"垂直"` ["vertical"]) -- since `_G.PreydatorL` is one flat table, only the later assignment ever took effect at runtime; the earlier one was silently dead weight, not a live alternate meaning. Deduplication kept the last-assigned (runtime-effective) value, `"垂直"`, and dropped the shadowed one -- this is a currently-shipping-behavior fix (the effective translation doesn't change, since Lua was already resolving to `"垂直"`), just removing a misleading, inert duplicate line, not overwriting a translator's actual choice. `koKR.lua` had 3 similar shadowed-duplicate keys (`Vertical`, `Above Bar`, `Below Bar`) but all with IDENTICAL values both times, so no behavior was ever at stake there, only redundancy.
