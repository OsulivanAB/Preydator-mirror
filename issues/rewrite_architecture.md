...
| HuntScanner: achievement signals/badges — built and live-confirmed 2026-09-01 (Decisions Log items 44-47). Tentatively closed: the `ACHIEVEMENT_EARNED` cache-wipe path (badge disappearing the moment an achievement is actually earned) is unverified since earning one is a single, non-repeatable event on the product owner's test account — flag if a stale badge is ever seen after an achievement completes | — | ✅ (tentative) |
...
| Slider value-number display in `UI/SettingsPanel.lua` — sliders (Font Size, Scale, Width, Height, Alert Cooldown, etc.) don't show their current numeric value, only the min/max endpoints; product owner flagged this live (2026-08-28) with screenshots. Needs checking whether Blizzard's native `Settings.CreateSliderOptions`/`CreateSlider` already supports a value label (may be a small config fix) or needs a custom value-text overlay | — | ✅ |
